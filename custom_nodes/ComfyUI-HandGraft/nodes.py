...
class ReplacePelvisWithDonor:
    ...
    def process(self, trimesh, genital_scale=1.10, voxel_body=False):
        ...
        # `-p <x> <y> <w> <h>` puts Blender's window FAR OFF-SCREEN. This is Blender's own flag, honored
        # even when it overrides the STARTUPINFO show-state -- so the window never appears on the visible
        # desktop, even while it sits "(not responding)" during the heavy GPU sculpt. The GL/GPU context
        # still works off-screen. Belt-and-suspenders with minimized + foreground-lock below.
        cmd = [BLENDER_EXE, "-p", "8000", "8000", "320", "240",
               "--python", _PELVIS_GRAFT, "--", "--body", fin, "--out", fout]
        if voxel_body:
