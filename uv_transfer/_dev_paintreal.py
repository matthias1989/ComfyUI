"""Paint the REAL path3 result onto the (resynced 157190) DEV blend from per-DEV-vertex bools:
blue=facemask, orange=concave, magenta=kept. Priority kept>concave>facemask>skin. Object-mode, solid=Material."""
...
fm   = np.load(UV+'/_dev_fm.npy').astype(bool)
con  = np.load(UV+'/_dev_con.npy').astype(bool)
kept = np.load(UV+'/_dev_kept.npy').astype(bool)
lv = np.empty(len(me.loops), dtype=np.int32); me.loops.foreach_get('vertex_index', lv)
ls = np.empty(nf, dtype=np.int32); lt = np.empty(nf, dtype=np.int32)
...
midx = np.zeros(nf, dtype=np.int32); midx[fmf]=1; midx[conf]=2; midx[kepf]=3
me.polygons.foreach_set('material_index', midx); me.update()
print('[real] facemask %d, concave %d, kept %d faces' % (int(fmf.sum()), int(conf.sum()), int(kepf.sum())))
for sc in bpy.data.screens:
    for ar in sc.areas:
        if ar.type=='VIEW_3D':
            for sp in ar.spaces:
                if sp.type=='VIEW_3D': sp.shading.type='SOLID'; sp.shading.color_type='MATERIAL'; sp.shading.light='FLAT'
bpy.ops.wm.save_as_mainfile(filepath=UV+'/last_seams_DEV.blend'); print('[real] saved')
