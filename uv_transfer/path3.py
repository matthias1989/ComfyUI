...
CX = float(os.environ.get('P2_FSUP_CX', '0.15'))   # lateral column width, in head-widths (tight => columns independent)
CZ = float(os.environ.get('P2_FSUP_CZ', '0.15'))   # height band, in head-widths
FSM = float(os.environ.get('P2_FSUP_M', '0.14')) * _hwest   # a neighbour this much more forward (head-rel) suppresses
cellx = CX * _hwest; cellz = CZ * _hwest
_gx = np.floor(_xz[:, 0] / cellx).astype(np.int64)
_gz = np.floor(_xz[:, 1] / cellz).astype(np.int64)
_cmin = {}
for _ii in range(len(_cvall)):
    _k = (int(_gx[_ii]), int(_gz[_ii]))
    if _yv[_ii] < _cmin.get(_k, 1e9):
        _cmin[_k] = float(_yv[_ii])
_dzr = int(os.environ.get('P2_FSUP_DZ', '0'))   # Z-coupling radius. 0 = each (x,z) cell independent. z+/-1 let a
#   more-forward crease one row UP (the sideburn/cheek above) suppress the under-ear hairline -- it cut charB's
#   under-ear in half (16/40 vs 29/40). The crown (the only thing z-coupling used to suppress) is now handled by
#   the deep-back Y-cap, so coupling is redundant. NO x-neighbour either -> lateral columns stay independent.
_lmin = np.empty(len(_cvall))
for _ii in range(len(_cvall)):
    _gxi, _gzi = int(_gx[_ii]), int(_gz[_ii]); _m = 1e9
    for _dz in range(-_dzr, _dzr + 1):
        _v = _cmin.get((_gxi, _gzi + _dz), 1e9)
        if _v < _m:
            _m = _v
    _lmin[_ii] = _m
_keep = (_yv - _lmin) <= FSM
front_v = set(int(v) for v in _cvall[_keep].tolist())
print('front-suppression (lateral-tight): kept %d/%d (cellx=%.4f cellz=%.4f M=%.4f hwest=%.4f)' % (
    int(_keep.sum()), len(_cvall), cellx, cellz, FSM, _hwest))
...
if int(os.environ.get('P2_HAIR_GUARD', '1')) and front_v and hair.any():
    _vadj2 = defaultdict(list)
    for (_a, _b) in e2f.keys():
        _vadj2[_a].append(_b); _vadj2[_b].append(_a)
    if int(os.environ.get('P2_BND_GUARD', '0')) and bnd_outer.any():   # default 0: GEO guard seeds from hair faces
        _near = set(int(v) for v in np.where(bnd_outer)[0].tolist())   # seed = the hair<->skin boundary edge
        _gname = 'boundary'
    else:
        _hairv = np.zeros(nv, bool)
        for _fi in np.where(hair)[0]:
            for _vv in fv[_fi]:
                _hairv[int(_vv)] = True
        _near = set(int(v) for v in np.where(_hairv)[0].tolist())     # seed = every hair face (old behaviour)
        _gname = 'hair-face'
    _n0g = len(front_v)
    if int(os.environ.get('P2_GUARD_GEO', '1')):
        # GEOMETRIC proximity (not topological rings): keep creases within distance D of the hair seed.
        # Topological dilation can't cross the EAR gap to reach the under-ear hairline (the mask lags it there,
        # and the ear separates them on the surface); a geometric ball does, because that line is only ~0.01
        # FORWARD of the hair. The eye/brow and chest/arm sit much farther from any hair -> still dropped.
        # D is head-relative -> self-calibrating, no per-character constant.
        _seedv = np.fromiter(_near, dtype=np.int64, count=len(_near))
        _Tg = cKDTree(co[_seedv])
        _D = float(os.environ.get('P2_GUARD_D', '0.40')) * _hwest   # generous: masks UNDER-detect hair (charB +X side),
        _fa = np.fromiter(front_v, dtype=np.int64, count=len(front_v))
        _dd, _ = _Tg.query(co[_fa])
        front_v = set(int(v) for v in _fa[_dd < _D].tolist())
        print('  (geometric guard D=%.4f = %.2f*head_width)' % (_D, _D / _hwest))
    else:
        _gfwd = int(os.environ.get('P2_GUARD_FWD', '1'))
        _yco = co[:, 1]
        for _ in range(int(os.environ.get('P2_HAIR_RINGS', '3'))):
            _nx = set()
            for v in _near:
                for w in _vadj2[v]:
                    if (not _gfwd) or _yco[w] <= _yco[v] + 1e-6:
                        _nx.add(w)
            _near |= _nx
        front_v &= _near
    print('proximity guard (%s seed): kept %d/%d creases near the hairline (dropped bare-skin + hair interior)' % (_gname, len(front_v), _n0g))
...
for i, v in enumerate(cv):
    ...
    k = int(zb[i])
    if k not in d or co[v, 1] < co[d[k], 1]:    # FRONT-most (min y) per (z-bin, side) -- self-calibrating,
        d[k] = int(v)                            # front-biased everywhere (the back is never picked = no back spots)
...
def _envelope(d):
    ...
    out = {}
    for k in ks:
        nb = [yk[j] for j in ks if abs(j - k) <= ENVW]
